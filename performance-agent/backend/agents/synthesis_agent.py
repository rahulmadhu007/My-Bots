"""Synthesis agent — autonomously cross-references specialist findings.

All AI-style analysis (correlation, recommendations, narrative, scoring)
is produced locally by this agent. No external API key is required.
"""
...
def _build_correlation_analysis(
    ...
    """Cross-reference datasets into one evidence-based paragraph."""
    signals: List[str] = []
...
def _build_recommendations(
    ...
    """Produce three prioritised, evidence-based recommendations."""
    ...
    # Strength / development recommendations when risks are light
    ...
    # Always ensure we have enough options
    defaults = [
...
def _build_summary_narrative(
    ...
    """Two-paragraph performance-review narrative."""
    sla = float(ticket_m.get("sla_met_pct") or 0)
    tat = float(ticket_m.get("avg_turnaround") or 0)
...
def synthesise(
    ...
    """Autonomously synthesise specialist results into a review package."""
    ...
    correlation = _build_correlation_analysis(
    ...
    print(
        f"[synthesis_agent] Autonomous synthesis complete for {employee_name} "
        f"(score={overall}, tier={tier})"
    )
    return result
